...
async function downloadBin(dest) {
  ...
  if (!token && !url) {
    throw new Error(
      "Missing IP2Location database. Run npm run download-db or set IP2LOCATION_TOKEN on Vercel."
    );
  }
...
async function resolveBinPath() {
  if (fs.existsSync(LOCAL_BIN) && fs.statSync(LOCAL_BIN).size > 1_000_000) {
    return LOCAL_BIN;
  }
  if (fs.existsSync(TMP_BIN) && fs.statSync(TMP_BIN).size > 1_000_000) {
    return TMP_BIN;
  }
